...
class Autodock():
	def __init__(self, python_path: str, utility_path: str, vina_path: str, receptor: str, dock_box:'DockBox'=None, hint:str=None, molinfo:str=None, radius:int=None):
		...
		if hint != None and os.path.exists(hint) and radius != None and radius >= 0 and molinfo != None:
			...
			raise Exception("Must specify docking coordinates through DockBox or hint/radius")
	...
	def sdf_to_pdbqt(sdf_file: str, pdbqt_file: str):
		...
		os.system(f"obabel {sdf_file} -O {pdbqt_file} 1>/dev/null")
	...
	@staticmethod
	def pdbqt_to_mol(pdbqt_path, mol_path):
		''' static method to convert pdbqt file to mol file using
			openbabel
		'''
		os.system(f"obabel {pdbqt_path} -O {mol_path}") 
	...
	def dock(self, ligprep_pdbqt: str, ligdock_pdbqt: str, lighighscore_pdbqt: str, lighighscore_pdb: str, exhaustiveness: int, num_modes: int, flex):
		...
		os.system(f"{self.vina_path} --config {config_path} --ligand {ligprep_pdbqt} --out {ligdock_pdbqt}")
